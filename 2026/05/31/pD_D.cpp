...
void solve(){
    ...
    for(int i = 0; i < q; i++){
        ...
        if(r - l < t) continue;
        int A=r-t,B=l+t - 1;
        qe.push_back({r, l, x, i});
        lisan.push_back(A), lisan.push_back(B);//, lisan.push_back(l + t - 1);
    }
    sort(ALL(lisan));
    ...
    for(auto [r, l, x, i] : qe){
        ...
        cout << bit.q(MN - 1) - bit.q(lower_bound(ALL(lisan), l + t - 1) - lisan.begin() + 1) << ' ' << i << '\n';
        cout << bit.q(lower_bound(ALL(lisan), l + t - 1) - lisan.begin() + 1)<< '\n';
        ans[i] = (bit.q(MN - 1) - bit.q(lower_bound(ALL(lisan), l + t - 1) - lisan.begin() + 1) >= x);
        /*while(ptr < n && worker[ptr][0] <= r - t + 1){
            //cout << "push " << worker[ptr][1] << '\n';
            bit.u(worker[ptr][1], 1);
            ptr++;
        }
        cout << bit.q(MN - 1) - bit.q(l + t - 1) << ' ' << i << '\n';
        //cout << lower_bound(ALL(lisan), l + t) - lisan.begin() << '\n';
        ans[i] = (bit.q(MN - 1) - bit.q(l + t - 1) >= x);*/
    }
